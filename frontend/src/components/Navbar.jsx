...
const Navbar = () => {
    ...
    const { setShowSearch, navigate, getCartCount, token, setToken } = useContext(ShopContext);
                    ...
                    {token &&
                        <div className='group-hover:block hidden absolute dropdown-menu right-0 pt-4'>
                            <div className='flex flex-col gap-2 w-36 py-3 px-5  bg-slate-100 text-gray-500 rounded'>
                                ...
                                <p onClick={() => { setToken(''); navigate('/login') }} className='cursor-pointer hover:text-black'>Logout</p>
                            </div>
                        </div>
                    }
                </div>
